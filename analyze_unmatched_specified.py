...
if __name__ == "__main__":
    print("--- Analyzing unmatched names from SpecifiedInvestment ---")
    ...
    # 3. すべてのユニークな名前を取得
    names_to_check = source_df["NameOfSecurities"].dropna().unique()
    if len(names_to_check) == 0:
        print("\n[Info] No names found in SpecifiedInvestment to check.")
        exit()
    ...
    matched_df = matching.match_names(all_names_series, master_df, score_cutoff=80)

    # 5. マッチングしなかった結果を抽出
    unmatched_df = matched_df[matched_df['matchedEdinetCode'].isnull()]

    # 6. 結果をファイルに保存
    output_path = "unmatched_names_specified.csv"
